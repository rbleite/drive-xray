...
TRANSLATIONS = {
    "pt": {
        ...
        "cleanup_or_run": "…ou executa o plano aqui, sem sair da app:",
        "cleanup_exec_btn": "▶️ Executar plano ({n} ficheiros · {size})",
        "cleanup_exec_title": "Confirmar execução",
        "cleanup_exec_q_warn": "Vão ser **movidos {n} ficheiros** ({size}) para a pasta de quarentena. **Nada é apagado** — podes recuperá-los enquanto não esvaziares a quarentena.",
        "cleanup_exec_d_warn": "Vão ser **APAGADOS permanentemente {n} ficheiros** ({size}). Esta acção **não tem retrocesso**.",
        "cleanup_exec_dest": "Destino da quarentena:",
        "cleanup_exec_keep_note": "Uma cópia de cada grupo é sempre mantida. Ficheiros com hardlinks para a cópia mantida não são tocados.",
        "cleanup_exec_recheck": "Cada ficheiro é reconfirmado no disco imediatamente antes de agir — o que tiver mudado desde a indexação é ignorado.",
        "cleanup_confirm_word": "SIM",
        "cleanup_exec_confirm": "Escreve {word} para confirmar",
        "cleanup_exec_go": "Executar agora",
        "cleanup_exec_running": "A executar… {rel}",
        "cleanup_exec_done": "✅ {n} ficheiros tratados · {size} libertados.",
        "cleanup_exec_skipped": "⏭️ {n} ignorados por terem mudado no disco desde a indexação.",
        "cleanup_exec_errors": "❌ {n} falharam.",
        "cleanup_exec_aborted": "🛑 Abortado sem tocar em nada: {reason}",
        "cleanup_exec_restore": "Para recuperar, move os ficheiros de volta a partir de `{path}`.",
        "cleanup_not_mounted": "🔌 A drive não está montada — liga-a para poderes executar o plano.",
        "cleanup_stale_hint": "🔄 O índice pode estar desatualizado. Faz refresh à drive antes de executar.",
        # interactive delete
    ...
    "en": {
        ...
        "cleanup_or_run": "…or run the plan here, without leaving the app:",
        "cleanup_exec_btn": "▶️ Run plan ({n} files · {size})",
        "cleanup_exec_title": "Confirm execution",
        "cleanup_exec_q_warn": "**{n} files** ({size}) will be **moved** to the quarantine folder. **Nothing is deleted** — you can restore them until you empty the quarantine.",
        "cleanup_exec_d_warn": "**{n} files** ({size}) will be **PERMANENTLY DELETED**. This **cannot be undone**.",
        "cleanup_exec_dest": "Quarantine destination:",
        "cleanup_exec_keep_note": "One copy of every group is always kept. Files hardlinked to the kept copy are never touched.",
        "cleanup_exec_recheck": "Each file is re-checked on disk immediately before acting — anything changed since indexing is skipped.",
        "cleanup_confirm_word": "YES",
        "cleanup_exec_confirm": "Type {word} to confirm",
        "cleanup_exec_go": "Run now",
        "cleanup_exec_running": "Running… {rel}",
        "cleanup_exec_done": "✅ {n} files handled · {size} freed.",
        "cleanup_exec_skipped": "⏭️ {n} skipped because they changed on disk since indexing.",
        "cleanup_exec_errors": "❌ {n} failed.",
        "cleanup_exec_aborted": "🛑 Aborted without touching anything: {reason}",
        "cleanup_exec_restore": "To restore, move the files back from `{path}`.",
        "cleanup_not_mounted": "🔌 The drive is not mounted — connect it to run the plan.",
        "cleanup_stale_hint": "🔄 The index may be out of date. Refresh the drive before running.",
        # interactive delete
